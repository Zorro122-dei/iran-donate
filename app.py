...
import qrcode, io, base64, time, os
...
# НАСТРОЙКИ РОСТА
BASE_VALUE = 18.4100  # Стартовый процент
GROWTH_PER_DAY = 0.35 # Сколько процентов прибавлять в сутки
START_DATE_FILE = "system_init.txt" # Файл, где хранится дата старта
...
def get_global_percent():
    ...
    # Читаем время запуска системы
    with open(START_DATE_FILE, "r") as f:
        start_time = float(f.read())
    
    # Считаем прошедшее время в днях
    diff_days = (time.time() - start_time) / 86400
    current_pct = BASE_VALUE + (diff_days * GROWTH_PER_DAY)
    
    return min(99.8542, current_pct) # Максимум 99.8%
...
<head>
    ...
    <style>
        body { 
            margin: 0; padding: 0; background: radial-gradient(circle, #3a3a3a 0%, #1a1a1a 100%); 
            color: #eee; font-family: 'Courier New', monospace; text-align: center; 
            overflow-x: hidden; min-height: 100vh; display: flex; flex-direction: column; align-items: center;
        }
        .ground-hole {
            position: fixed; bottom: -60px; width: 140%; height: 200px;
            background: radial-gradient(ellipse at center, #050505 0%, #1a1a1a 70%);
            border-radius: 50% 50% 0 0; z-index: 1; box-shadow: inset 0 30px 60px #000;
        }
        .rocket-box { position: fixed; top: 120px; width: 45px; height: 300px; z-index: 1; }
        .rocket-body {
            width: 40px; height: 200px; background: linear-gradient(90deg, #333 0%, #666 50%, #222 100%);
        ...
        .rocket-nose {
            ...
            background: linear-gradient(90deg, #800 0%, #f33 50%, #600 100%); border-radius: 50% 50% 20% 20%;
        }
        ...
        .manif { border-left: 4px solid #f05; background: rgba(0,0,0,0.85); padding: 15px; margin: 20px auto; text-align: left; color: #ccc; border: 1px solid #333; font-size: 13px; }
        .goal-bg { width: 300px; height: 12px; border: 1px solid #0ff; margin: 15px auto; position: relative; background: #000; overflow: hidden; }
        .goal-up { height: 100%; background: #0ff; box-shadow: 0 0 15px #0ff; width: {{ pct }}%; }

        .wrap { display: flex; flex-wrap: wrap; justify-content: center; gap: 30px; padding: 20px; position: relative; z-index: 5; }
        .card { background: #111; border: 1px solid #444; padding: 25px; width: 260px; transition: all 0.3s ease; cursor: pointer; }
        ...
        .btn { border: 1px solid #0ff; color: #0ff; background: transparent; padding: 12px; width: 100%; font-weight: bold; cursor: pointer; }
