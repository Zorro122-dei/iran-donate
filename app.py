...
<head>
    ...
    <style>
        ...
        .goal-up { height:100%; background:#0ff; box-shadow:0 0 10px #0ff; transition: width 2s ease-out; }
        ...
        .addr { font-size:10px; word-break:break-all; color:#888; margin-bottom:15px; padding:5px; }
...
<body>
    ...
    <div class="wrap">
        ...
        <div class="card">
            ...
            <div class="qr"><img src="data:image/png;base64,{{ g_func(w.a) }}" width="100%"></div>
            <div class="addr" id="copy-{{ loop.index }}">{{ w.a }}</div>
            <button class="btn" onclick="cp('copy-{{ loop.index }}')">COPY ADDRESS</button>
        </div>
        {% endfor %}
    </div>
    <div id="tx-box">
    ...
    <script>
        ...
        // Динамическая шкала
        let base = 18.41;
        let randomAdd = Math.random() * 0.05; // Добавляем чуть-чуть при каждом заходе
        let final = base + randomAdd;
        document.getElementById('p').innerText = final.toFixed(2);
        setTimeout(() => { document.getElementById('f').style.width = final + '%'; }, 500);
...
@app.route('/')
def i(): return render_template_string(H, W=W, g_func=g)
